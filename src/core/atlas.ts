...
import { plan, propose, critique, isFormControl, memory, criticAgent } from "../agents/index.js";
...
export async function runAtlas(goal: string, startUrl: string, opts: AtlasOptions = {}): Promise<AtlasRunArtifacts> {
  ...
  try {
    ...
    for (let t = 0; t < maxSteps; t++) {
      ...
      logDebug("Lookahead states prepared", { step: t, lookaheads, uncertainties });

      const critiqueRes = await critique(goal, P, o, C, lookaheads, t, onEvent, recentActions, uncertainties);
      ...
      const delta = await generateDelta(criticAgent, o, choice.action, oNext);
      ...
      if (onEvent) {
        ...
        if (transition) {
          await onEvent({ type: "map_update", step: t, edge: clone(transition) });
        }
      }
